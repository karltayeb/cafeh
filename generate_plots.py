...
def generate_plots(path, name)
    ...
    gene_path = path.split('/')
    gene_path[1] = 'GTEx_genes'
    gene_path = '/'.join(save_path)
    print(gene_path)


    correlation_path = path.split('/')
    correlation_path[1] = 'correlation_matrices'
    correlation_path = '/'.join(save_path)
...
for path, subdirs, files in os.walk('./model-dicts/'):
    for name in files:
